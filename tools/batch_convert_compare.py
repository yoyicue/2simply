...
def process_single_file(args: Tuple[Path, Path, Path, bool]) -> Tuple[str, bool, str]:
    ...
    try:
        ...
        os.system(f'python converter/json2musicxml.py --input "{input_file}" --output "{temp_musicxml}" > /dev/null 2>&1')
        
        # 第二步：MusicXML到JSON的转换
        os.system(f'python converter/musicxml2json.py --input "{temp_musicxml}" --output "{output_json}" > /dev/null 2>&1')
        
        # 第三步：比较结果
        result = os.system(f'python tools/score_compare.py "{input_file}" "{output_json}" --quiet > /dev/null 2>&1')
        
        # 如果不保留输出且文件在临时目录，则清理临时文件
        if not keep_output and temp_musicxml.parent == temp_dir and temp_musicxml.exists():
            temp_musicxml.unlink()
            
        return str(input_file), result == 0, ""
...
def main():
    ...
    try:
        # 设置目录
        base_path, temp_dir, output_dir = setup_directories(args.cache_dir)
